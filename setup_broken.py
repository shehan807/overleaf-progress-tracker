...
def setup(repo_path, github_token, repo_name):
    ...
    try:
        ...
        click.echo(f" Successfully authenticated with GitHub and found repository: {repo.full_name}")
    ...
    if workflow_dest.exists():
        if not click.confirm(f"Workflow file already exists at {workflow_dest}. Overwrite?"):
            ...
            click.echo(f" Copied workflow file to {workflow_dest}")
    else:
        shutil.copy2(workflow_template, workflow_dest)
        click.echo(f" Created workflow file at {workflow_dest}")
    ...
    if not progress_file.exists():
        ...
        click.echo(f" Initialized progress data file at {progress_file}")
    ...
    click.echo(f" Copied tracking scripts to {scripts_dest}")
